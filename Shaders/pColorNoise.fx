...
float3 ColorNoisePass(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	float weight = Strength / (luma * 2 + 2.0); //Multiply luma by 2 to simulate a wider dynamic range than is actually present and divide Strength by 2 to reduce sensitivity and set maximum SNR to 50%.
